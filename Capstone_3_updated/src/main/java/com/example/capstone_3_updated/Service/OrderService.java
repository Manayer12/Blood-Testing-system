...
import com.example.capstone_3_updated.Repository.OrderRepository;
...
public class OrderService {
    ...
    public void cancle_appointment(Integer order_id){
        ...
        if (user.getTestType().getAppointment() && Objects.equals(orders.getOrder_status(), "processing")) {
            testType.setAppointment(false);
            user.setTestType(null);
            orderRepository.delete(orders);
        }
        else if(user.getTestType().getAppointment() && Objects.equals(orders.getOrder_status(), "confirmed")) {
